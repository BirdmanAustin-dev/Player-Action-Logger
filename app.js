...
let moderationState = {enabled: false, tokenRequired: true, defaultTemporaryBanMinutes: 60, maximumTemporaryBanMinutes: 43200, temporaryBans: []};
let moderationScheduleMode = 'duration';
...
document.querySelectorAll('[data-moderation-action]').forEach(button => button.addEventListener('click', () => selectModerationAction(button.dataset.moderationAction)));
document.querySelectorAll('[data-schedule-mode]').forEach(button => button.addEventListener('click', () => setModerationScheduleMode(button.dataset.scheduleMode)));
document.querySelectorAll('[data-duration-minutes]').forEach(button => button.addEventListener('click', () => setDurationFromMinutes(Number(button.dataset.durationMinutes))));
$('moderationDurationAmount').addEventListener('input', updateModerationDurationPreview);
$('moderationDurationUnit').addEventListener('change', updateModerationDurationPreview);
$('moderationUnbanAt').addEventListener('change', updateModerationDurationPreview);
...
function activityCard(player) {
  ...
  return `<article class="activity-card">
    ...
    <ul>${detail}</ul>
    <div class="mini-stats"><span><b>${player.blocksPlaced}</b> placed</span><span><b>${player.blocksBroken}</b> broken</span><span><b>${player.pvpEvents}</b> PvP participation</span></div>
...
function renderPlayers() {
  ...
  $('playerProfiles').innerHTML = rows.length ? rows.map(name => {
    ...
    return `<article class="profile-card">
      ...
      <div class="profile-quick-stats"><span><b>${activity?.blocksPlaced || 0}</b> placed</span><span><b>${activity?.blocksBroken || 0}</b> broken</span><span><b>${activity?.pvpEvents || 0}</b> PvP participation</span></div>
...
async function openProfile(name) {
  ...
  try {
    ...
    $('profileModalBody').innerHTML = `
      ...
      <div class="profile-stat-grid">${profileStat('Events', stats.totalEvents)}${profileStat('Placed', stats.blocksPlaced)}${profileStat('Broken', stats.blocksBroken)}${profileStat('PvP participation', stats.pvpEvents)}${profileStat('PvE', stats.pveEvents)}${profileStat('Areas', stats.activeAreas)}</div>
...
function renderHistoryDay(data) {
  ...
  const activities = Object.entries(summary.activities || {});
...
function alertRows(alerts, compact) {
  ...
  return alerts.map(a => {
    ...
    return `<article class="alert-row ${String(a.severity).toLowerCase()} ${compact ? 'compact' : ''} ${open ? '' : 'closed'}" onclick="openAlertEvidence('${escapeJs(a.id)}')">
...
async function openAlertEvidence(id) {
  if ($('profileModal').open) $('profileModal').close();
  if ($('combatModal').open) $('combatModal').close();
  $('logModalTitle').textContent = 'Alert evidence';
  $('logModalBody').innerHTML = '<div class="empty-state">Loading focused evidence…</div>';
  $('logModal').showModal();
  try {
    const data = await apiFetch('/api/alerts/evidence?id=' + encodeURIComponent(id));
    if (data.evidenceType === 'combat') {
      $('logModal').close();
      openCombat(data.evidenceId);
      return;
    }
    const rows = data.evidence || [];
    const countText = Number(data.matchingLines || rows.length);
    $('logModalTitle').textContent = data.title || 'Alert evidence';
    $('logModalBody').innerHTML = `
      <section class="alert-evidence-summary">
        <div><span class="eyebrow">${escapeHtml(data.player || '')} • ${escapeHtml(data.timeLabel || '')}</span><h3>${escapeHtml(data.title || '')}</h3><p>${escapeHtml(data.detail || '')}</p></div>
        <button class="secondary-button" onclick="loadLogs('${escapeJs(data.player)}')">Open full player log</button>
      </section>
      <div class="evidence-note">Showing ${countText.toLocaleString()} matching evidence line${countText === 1 ? '' : 's'} from the alert's ${Number(data.windowMinutes || 0)}-minute activity window. The newest ${rows.length} are displayed when the result is large.</div>
      <div class="raw-log alert-evidence-log">${rows.map(row => `<div><time>${escapeHtml(row.time)}</time><span>${escapeHtml(row.action)}</span></div>`).join('') || '<div class="empty-state compact">No matching lines were found; nearby log lines are shown when available.</div>'}</div>`;
  } catch (error) {
...
function openModeration(player, action = 'kick', reason = '', alertId = '') {
  ...
  $('moderationTargetName').textContent = player;
  ...
  setDurationFromMinutes(Number(moderationState.defaultTemporaryBanMinutes || 60), false);
  setModerationScheduleMode('duration');
  selectModerationAction(action);
  $('moderationModal').showModal();
}

function selectModerationAction(action) {
  const allowed = ['kick', 'tempban', 'ban', 'pardon'];
  const selected = allowed.includes(action) ? action : 'kick';
  $('moderationAction').value = selected;
  document.querySelectorAll('[data-moderation-action]').forEach(button => {
    button.classList.toggle('active', button.dataset.moderationAction === selected);
  });
  const labels = {
    kick: 'Kick player',
    tempban: 'Apply temporary ban',
    ban: 'Permanently ban player',
    pardon: 'Pardon player'
  };
  $('moderationSubmit').textContent = labels[selected];
  $('moderationSubmit').classList.toggle('danger', selected === 'ban');
  updateModerationDurationVisibility();
}

function updateModerationDurationVisibility() {
  const temporary = $('moderationAction').value === 'tempban';
  $('tempBanSchedule').classList.toggle('hidden', !temporary);
  if (temporary) updateModerationDurationPreview();
}

function setModerationScheduleMode(mode) {
  moderationScheduleMode = mode === 'date' ? 'date' : 'duration';
  document.querySelectorAll('[data-schedule-mode]').forEach(button => {
    button.classList.toggle('active', button.dataset.scheduleMode === moderationScheduleMode);
  });
  $('durationControls').classList.toggle('hidden', moderationScheduleMode !== 'duration');
  $('unbanDateField').classList.toggle('hidden', moderationScheduleMode !== 'date');
  if (moderationScheduleMode === 'date' && !$('moderationUnbanAt').value) {
    const defaultMinutes = Number(moderationState.defaultTemporaryBanMinutes || 60);
    $('moderationUnbanAt').value = localDateTimeValue(new Date(Date.now() + defaultMinutes * 60000));
  }
  updateModerationDurationPreview();
}

function setDurationFromMinutes(minutes, update = true) {
  const safeMinutes = Math.max(1, Math.round(Number(minutes) || 60));
  if (safeMinutes % 1440 === 0) {
    $('moderationDurationAmount').value = safeMinutes / 1440;
    $('moderationDurationUnit').value = 'days';
  } else if (safeMinutes % 60 === 0) {
    $('moderationDurationAmount').value = safeMinutes / 60;
    $('moderationDurationUnit').value = 'hours';
  } else {
    $('moderationDurationAmount').value = safeMinutes;
    $('moderationDurationUnit').value = 'minutes';
  }
  moderationScheduleMode = 'duration';
  document.querySelectorAll('[data-schedule-mode]').forEach(button => {
    button.classList.toggle('active', button.dataset.scheduleMode === 'duration');
  });
  $('durationControls').classList.remove('hidden');
  $('unbanDateField').classList.add('hidden');
  if (update) updateModerationDurationPreview();
}

function selectedDurationMinutes() {
  const amount = Number($('moderationDurationAmount').value);
  const multipliers = {minutes: 1, hours: 60, days: 1440};
  return Math.round(amount * (multipliers[$('moderationDurationUnit').value] || 1));
}

function updateModerationDurationPreview() {
  if ($('moderationAction').value !== 'tempban') return;
  const maximum = Number(moderationState.maximumTemporaryBanMinutes || 43200);
  let expiresAt;
  let durationMinutes;
  if (moderationScheduleMode === 'date') {
    expiresAt = new Date($('moderationUnbanAt').value).getTime();
    durationMinutes = Math.ceil((expiresAt - Date.now()) / 60000);
  } else {
    durationMinutes = selectedDurationMinutes();
    expiresAt = Date.now() + durationMinutes * 60000;
  }
  if (!Number.isFinite(durationMinutes) || durationMinutes < 1 || !Number.isFinite(expiresAt)) {
    $('moderationDurationPreview').textContent = 'Choose a valid future duration or date.';
    $('moderationDurationPreview').classList.add('invalid');
    return;
  }
  const tooLong = durationMinutes > maximum;
  $('moderationDurationPreview').textContent = tooLong
    ? `This exceeds the configured maximum of ${formatDuration(maximum)}.`
    : `Automatic pardon: ${new Date(expiresAt).toLocaleString()} • ${formatDuration(durationMinutes)}`;
  $('moderationDurationPreview').classList.toggle('invalid', tooLong);
}
...
function formatDuration(minutes) {
  ...
  if (hours) return `${hours} hour${hours === 1 ? '' : 's'}${mins ? ` ${mins} minute${mins === 1 ? '' : 's'}` : ''}`;
  ...
  if (action === 'tempban') {
    if (moderationScheduleMode === 'date') {
      ...
      if (!Number.isFinite(expiresAt) || expiresAt <= Date.now()) {
        $('moderationResult').textContent = 'Choose an unban date in the future.';
        return;
      }
    } else {
      durationMinutes = selectedDurationMinutes();
      if (!Number.isFinite(durationMinutes) || durationMinutes < 1) {
        $('moderationResult').textContent = 'Choose a valid temporary-ban duration.';
        return;
      }
    }
    const effectiveMinutes = expiresAt > 0 ? Math.ceil((expiresAt - Date.now()) / 60000) : durationMinutes;
    const maximum = Number(moderationState.maximumTemporaryBanMinutes || 43200);
    if (effectiveMinutes > maximum) {
      $('moderationResult').textContent = `The configured maximum is ${formatDuration(maximum)}.`;
      return;
    }
  }
...
function renderInsights() {
  ...
  $('statisticsCards').innerHTML = [
    ...
    statCard('Combat records', (s.pvpEvents || 0) + (s.pveEvents || 0), `${s.pvpEvents || 0} PvP participation • ${s.pveEvents || 0} PvE`, 'red', '⚔'),
  ...
  renderBarChart('eventChart', {
    ...
    'PvP participation records': s.pvpEvents || 0, 'PvE events': s.pveEvents || 0,
    'Farming': s.farmingEvents || 0, 'Fishing': s.fishingEvents || 0,
...
function openEvidence(type, id) {
  ...
  else if (type === 'activity') openActivityEvidence(id);
  else loadLogs(id);
}

async function openActivityEvidence(id) {
  if ($('profileModal').open) $('profileModal').close();
  $('logModalTitle').textContent = 'Activity evidence';
  $('logModalBody').innerHTML = '<div class="empty-state">Loading selected activity…</div>';
  $('logModal').showModal();
  try {
    const data = await apiFetch('/api/activity-evidence?id=' + encodeURIComponent(id));
    $('logModalTitle').textContent = data.item.title;
    $('logModalBody').innerHTML = '<div class="evidence-note">' + escapeHtml(data.item.detail)
      + '</div><div class="raw-log">' + data.records.map(row => '<div><time>'
      + escapeHtml(row.time) + '</time><span>' + escapeHtml(row.action) + '</span></div>').join('') + '</div>';

...
const categoryTone = {
  ...
  'Fishing': 'cyan', 'Items / storage': 'cyan', 'Crafting / utility': 'amber',
...
function activityCard(player) {
  ...
  return `<article class="activity-card">
    ...
    <div class="card-actions"><button onclick="openProfile('${escapeJs(player.name)}')">Profile</button><button class="subtle" onclick="openHistory('${escapeJs(player.name)}')">History</button><button class="subtle" onclick="loadLogs('${escapeJs(player.name)}')">Raw log</button></div>
  </article>`;
}
...
function renderPlayers() {
  ...
  $('playerProfiles').innerHTML = rows.length ? rows.map(name => {
    ...
    return `<article class="profile-card">
      ...
      <div class="card-actions">${activity ? `<button onclick="openProfile('${escapeJs(name)}')">Profile</button>` : ''}<button class="subtle" onclick="openHistory('${escapeJs(name)}')">History</button><button class="subtle" onclick="loadLogs('${escapeJs(name)}')">Raw log</button></div>
    </article>`;
...
function renderHistoryDay(data) {
  ...
  $('historyDayDetail').innerHTML = `
    <div class="history-hero"><div><span class="eyebrow">${escapeHtml(data.player)} • ${escapeHtml(summary.date)}</span><h3>${escapeHtml(summary.dominantActivity || 'Activity summary')}</h3><p>${Number(summary.totalEvents || 0).toLocaleString()} events from ${escapeHtml(summary.firstSeen || '')} to ${escapeHtml(summary.lastSeen || '')}</p></div><button class="secondary-button" onclick="loadLogs('${escapeJs(data.player)}')">Open full raw log</button></div>

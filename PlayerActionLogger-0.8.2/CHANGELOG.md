...
- Added durable moderation alerts stored in `moderation-alerts.json`. Alerts remain open until an administrator reviews and closes them.
- Added dashboard alert controls for opening evidence, marking an alert closed, and starting an investigation temporary ban.
- Added audited dashboard moderation actions: kick, permanent ban, temporary ban, and pardon.
- Added persistent temporary-ban timers in `temporary-bans.json`, dashboard countdowns, restart survival, and automatic `pardon` when time expires.
- Privileged dashboard actions require a non-empty dashboard access token by default.
- Added login, logout, and kick locations to username logs.
- Reworked inventory logging to focus on storage containers instead of ordinary player-inventory rearranging.
- Added `CONTAINER_PUT`, `CONTAINER_TAKE`, `CONTAINER_MOVE`, and `CONTAINER_DRAG` evidence for chests, barrels, shulker boxes, hoppers, dispensers, droppers, and ender chests.
- Removed pickup/drop events from the live `Items / storage` classification so routine item collection does not masquerade as inventory management.
- Added token-protected deletion of a selected player-history date from the dashboard.
- Added moderation-action and history-deletion audit entries to `_server.log`.

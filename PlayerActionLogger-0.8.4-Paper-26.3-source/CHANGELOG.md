# PlayerActionLogger 0.8.4 — Paper 26.3

- Target Paper API 26.3.build.28-alpha and Java 25; update plugin metadata and output filename.
- Fix Overview, Activity Feed, and profile recent-activity clicks opening the full player log.
- Preserve event-specific log timestamps/actions in the bounded rolling activity model.
- Add authenticated activity-evidence endpoint and focused evidence dialog.
- Include world and chunk in group identifiers to prevent cross-location evidence collisions.
- Label unlogged activity observations and report expired activity links clearly.
- Revalidate dashboard assets so updated event-link behavior loads after upgrades.
- Four Java and four dashboard regression tests passed; no live-server test performed.
